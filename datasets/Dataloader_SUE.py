...
class Dataloader_SUE(Dataset):
    def __init__(self,root,transforms,names=['satellite','drone']):
        super(Dataloader_SUE).__init__()
...
def train_collate_fn_sue(batch):
    img_s,img_st,img_d,ids = zip(*batch)
    ids = torch.tensor(ids,dtype=torch.int64)
    return [torch.stack(img_s, dim=0),ids],[torch.stack(img_s, dim=0),ids], [torch.stack(img_d,dim=0),ids]
...
if __name__ == '__main__':
    ...
    datasets = Dataloader_SUE(root="E:/crossview/code/data/train",transforms=transform_train_list,names=['satellite','drone'])
    samper = Sampler_SUE(datasets,8,2)
    dataloader = DataLoader(datasets,batch_size=8,num_workers=0,sampler=samper,collate_fn=train_collate_fn_sue)
