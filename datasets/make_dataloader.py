...
from .Dataloader_University import Sampler_University,Dataloader_University,train_collate_fn_university
from .Dataloader_SUE import Sampler_SUE,Dataloader_SUE,train_collate_fn_sue
from .random_erasing import RandomErasing
from .autoaugment import ImageNetPolicy, CIFAR10Policy
import torch

def make_dataset(opt):
    if opt.dataset.lower() == "sue":
        ######################################################################
        # Load Data
        # ---------
        #
        ...
        image_datasets = Dataloader_SUE(opt.data_dir,transforms=data_transforms)
        samper = Sampler_SUE(image_datasets,batchsize=opt.batchsize,sample_num=opt.sample_num,triplet_loss=opt.triplet_loss)
        dataloaders =torch.utils.data.DataLoader(image_datasets, batch_size=opt.batchsize,sampler=samper,num_workers=0, pin_memory=True,collate_fn=train_collate_fn_sue)
        ...
        dataloaders =torch.utils.data.DataLoader(image_datasets, batch_size=opt.batchsize,sampler=samper,num_workers=0, pin_memory=True,collate_fn=train_collate_fn_university)
